...
from database.test_session import session
...
def create_todo(session:Session,todo:ToDoRequest)->str:
    ...
    session.add(new_todo) # db에 알아서 insert id가 None
    # print('★ commit 전 데이터의 id :', new_todo.id)
    session.commit() # commit하면 db에있는 id로 들어감
...
if __name__=='__main__':
    ...
    print('2. 상세 :', get_todo(session, 1)) # 있는 데이터
    print('2. 상세 :', get_todo(session, 9)) # 없는 데이터
    todo = ToDoRequest()
