...
public class Ex2_selectWhere {
	public static void main(String[] args) {
		...
		try {
			...
			rs = stmt.executeQuery(query1); // 4,5. SQL문 전송 + 결과 받기
			if(rs.next()) { // 부서번호가 있는 경우
				System.out.println("원하는 부서번호는 : " + deptno);
				...
				if(rs.next()) { // 사원이 존재할 경우
					...
					System.out.println(deptno + "사원이 존재하지 않습니다.");
				}
			}else { // 부서번호가 없는 경우
				System.out.println(deptno +"번은 유효한 부서번호가 아닙니다");				
			}
